...
- 제목: "기존 서비스는 여기서 멈춘다"
- 비교표 (5행):
  서비스        | MAU        | 알 수 있는 것          | 빠진 것
  직방          | 229만      | 매물 가격·면적         | 동네 분위기·성격
  호갱노노      | 179만      | 실거래가 시세 흐름      | 라이프스타일 적합성
  카카오맵      | —          | 주변 시설 위치          | 데이터 기반 동네 정의
  기존 앱 공통   | —          | 앱 정보-실제 일치율 41% (한국소비자원) | —
  동네 MBTI     | —          | 위 모두 + AI 동네 성격 + 이사 타이밍 | (없음)
- 마지막 행은 #29B5E8 배경 강조 처리
- 하단: "Snowflake Cortex AI로 '동네 적합성 판단'을 처음 구현"
- 출처: 와이즈앱 2024.01, 한국소비자원 2018
...
[향후 실행 방안 제안] — 파란 헤더바, 흰 박스
  · 사용자 본인 MBTI 입력 → 궁합 동네 자동 매칭 추천 (Jokela 2015 Person-Environment Fit 기반)
...
[기대 효과] — 파란 헤더바, 흰 박스
  · TAM: 연 43만 명(서울 전입) + 79만(서울 내 이동) = 122만 건/연
  · 한국 PropTech 시장 2.3조 원 (5년 8배 성장), 부동산 앱 MAU 645만+
  · 부동산 앱 이용자 44%가 2030세대 → 동네 MBTI 타겟 사용자 직결
