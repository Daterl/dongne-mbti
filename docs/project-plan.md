...
**정량 근거**: [통계청 국내인구이동통계(2024)](https://data.si.re.kr/smr2024/smr2024_01population.html) 기준 매년 약 43만 명이 타 시도에서 서울로 전입. 서울 내 주거 이동까지 포함하면 연간 약 122만 건. [한국리서치 공원이용현황조사(2025)](https://scpm.seoul.go.kr/seoul-policy/evt0236) 에 따르면 주거지 선택 시 '동네 분위기·생활환경'을 중요하게 보는 비율이 78%로, 교육환경(60%)을 크게 앞섬. 그러나 기존 서비스는 가격·교통 정보만 제공한다.
*(추가 근거: 2024년 서울 전입 사유 중 '주택' 34.5%, '직업' 21.7% — 주거 환경이 이사 결정의 핵심 요인임을 방증. 출처: 통계청 2024년 국내인구이동통계)*
...
- **동네 MBTI**: Snowflake Marketplace 4종(상권·부동산·역세권·인구) 크로스 분석 → AI가 동네의 '성격'을 MBTI 16유형으로 정의. 자연어로 "나에게 맞는 동네"를 찾고, ML로 이사 타이밍까지 예측. **기존 서비스가 제공하지 않는 '동네 적합성' 판단을 AI로 처음 구현**.
  *(학술 선행 연구: Jokela et al., 2015, "Geographically varying associations between personality and life satisfaction in the USA", J. Research in Personality — 거주지·지역과 성격 특성의 통계적 상관관계를 실증. 본 프로젝트는 이를 Snowflake Cortex AI로 한국 서울 동 단위 정량화·상용화한 응용 구현)*
